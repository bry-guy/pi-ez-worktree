...
export async function attachWorktree({ cwd, target }) {
	...
	if (normalizedTarget) {
		...
		if (!match && candidates.length === 1) match = candidates[0];
			...
			throw new Error(`Multiple linked worktrees are available. Re-run /ezwt attach with a branch or path.\n\nCandidates:\n${summary}`);
		}
	}
...
export async function finishWorktree(state, options = {}) {
	...
	if (initialStatus.rebaseInProgress || initialStatus.mergeInProgress) {
		return {
			status: "blocked",
			message: "Worktree already has a merge or rebase in progress. Resolve that first, then rerun /ezwt finish.",
	...
	if (strategy === "auto") {
		...
		if (rebaseResult.code !== 0) {
			return {
				status: "conflict",
				message: "Rebase hit conflicts inside the active worktree. Resolve them in this pi session, then rerun /ezwt finish.",
...
export function formatAttachableStatusText({ repoRoot, currentTopLevel, candidates }) {
	...
	if (candidates.length === 1) {
		...
		lines.push(`hint: run /ezwt attach ${getWorktreeAttachTarget(candidate)}`);
	} else {
		lines.push("hint: run /ezwt attach <branch-or-path>");
	}
	if (resolve(currentTopLevel) !== resolve(repoRoot)) {
		lines.push(`current checkout: ${currentTopLevel}`);
	}
