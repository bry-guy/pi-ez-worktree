...
import { access, glob, lstat, mkdir, readFile, readlink, symlink, writeFile } from "node:fs/promises";
...
const CONFIG_FILE = ".ez-worktree.json";
...
async function ensureMetadataIgnored(worktreePath) {
	const excludePathRaw = await gitStdout(worktreePath, ["rev-parse", "--git-path", "info/exclude"]);
	const excludePath = isAbsolute(excludePathRaw) ? excludePathRaw : resolve(worktreePath, excludePathRaw);
	await mkdir(dirname(excludePath), { recursive: true });
	const existing = (await readFile(excludePath, "utf8").catch(() => "")).split(/\r?\n/).map((line) => line.trim());
	if (existing.includes(METADATA_FILE)) return;
	const prefix = existing.filter(Boolean).length > 0 ? "\n" : "";
	await writeFile(excludePath, `${await readFile(excludePath, "utf8").catch(() => "")}${prefix}${METADATA_FILE}\n`, "utf8");
}

async function writeWorktreeMetadata(state) {
	await ensureMetadataIgnored(state.worktreePath);
...
async function readEzWorktreeConfig(repoRoot) {
	const content = await readFile(configPath(repoRoot), "utf8").catch((error) => {
		if (error?.code === "ENOENT") return undefined;
		throw error;
	});
	if (!content) return { include: [], includeOptional: [] };
	let parsed;
	try {
	...
	const include = Array.isArray(parsed?.include) ? parsed.include.map(normalizeIncludePattern).filter(Boolean) : [];
	const includeOptional = Array.isArray(parsed?.includeOptional) ? parsed.includeOptional.map(normalizeIncludePattern).filter(Boolean) : [];
	return { include, includeOptional };
}

function sourcePathForGlobEntry(sourceRoot, entry) {
	return isAbsolute(entry) ? resolve(entry) : resolve(sourceRoot, entry);
}

function destinationRelativePath(sourceRoot, sourcePath) {
	const repoRelativePath = relative(sourceRoot, sourcePath);
	if (repoRelativePath && !repoRelativePath.startsWith("..") && !isAbsolute(repoRelativePath)) return repoRelativePath;
	return basename(sourcePath);
}

async function expandIncludePatterns(sourceRoot, patterns, optional = false) {
	const files = [];
	for (const pattern of patterns) {
		let matchedFile = false;
		for await (const entry of glob(pattern, { cwd: sourceRoot })) {
			const sourcePath = sourcePathForGlobEntry(sourceRoot, entry);
			const stat = await lstat(sourcePath).catch(() => undefined);
			if (!stat || stat.isDirectory()) continue;
			matchedFile = true;
			files.push({ pattern, optional, sourcePath, destinationRelativePath: destinationRelativePath(sourceRoot, sourcePath) });
		}
		if (!matchedFile && !optional) files.push({ unmatched: pattern, optional });
	}
	return files;
}

function includeConflictMessage(destinationRelativePath) {
	return `Skipped include because destination already exists: ${destinationRelativePath}`;
}

async function applyConfiguredIncludes({ sourceRoot, worktreePath, config }) {
	config ??= await readEzWorktreeConfig(sourceRoot);
	const result = { configFile: CONFIG_FILE, included: [], warnings: [] };
	if (config.include.length === 0 && config.includeOptional.length === 0) return result;
	const expanded = [
		...(await expandIncludePatterns(sourceRoot, config.include, false)),
		...(await expandIncludePatterns(sourceRoot, config.includeOptional, true)),
	];
	const plannedDestinations = new Set();
	for (const entry of expanded) {
		if (entry?.unmatched) {
			result.warnings.push(`No files matched include pattern: ${entry.unmatched}`);
			continue;
		}
		const { sourcePath, destinationRelativePath, optional } = entry;
		if (plannedDestinations.has(destinationRelativePath)) {
			const message = `Skipped include because another include already targets destination: ${destinationRelativePath}`;
			if (optional) {
		...
		const destinationPath = join(worktreePath, destinationRelativePath);
		const existing = await lstat(destinationPath).catch(() => undefined);
		if (existing) {
			if (existing.isSymbolicLink()) {
				const target = await readlink(destinationPath).catch(() => undefined);
				if (target === sourcePath) {
					plannedDestinations.add(destinationRelativePath);
					result.included.push(destinationRelativePath);
					continue;
				}
			}
			const message = includeConflictMessage(destinationRelativePath);
			if (optional) {
				result.warnings.push(message);
				continue;
			}
...
function attachIncludeResult(state, includeResult) {
	Object.defineProperty(state, "includeResult", {
		value: includeResult,
		enumerable: false,
		configurable: true,
	});
...
export function formatIncludeResultText(includeResult) {
	if (!includeResult) return "";
	const lines = [];
	if (includeResult.included.length > 0) {
		lines.push(`Included ${includeResult.included.length} file${includeResult.included.length === 1 ? "" : "s"} from ${includeResult.configFile}.`);
	}
	if (includeResult.warnings.length > 0) {
		lines.push(`Include warnings:\n${includeResult.warnings.map((warning) => `- ${warning}`).join("\n")}`);
	}
	return lines.join("\n");
}
...
export async function createWorktree({ cwd, name, branchPrefix = "pi" }) {
	...
	const config = await readEzWorktreeConfig(repoRoot);
	...
	try {
		const includeResult = await applyConfiguredIncludes({ sourceRoot: repoRoot, worktreePath: candidatePath, config });
		return attachIncludeResult(state, includeResult);
	} catch (error) {
		await runGit(repoRoot, ["worktree", "remove", "--force", candidatePath], { allowFailure: true });
		await runGit(repoRoot, ["branch", "-D", candidateBranch], { allowFailure: true });
		throw error;
	}
}

export async function listAttachableWorktrees(cwd) {
	...
	const includeCurrent = currentMetadata?.worktreePath && resolve(currentMetadata.worktreePath) === resolve(currentTopLevel);
	const mainCheckoutRoot = resolve(currentMetadata?.mainCheckoutPath || currentTopLevel);
	const candidates = worktrees.filter((entry) => {
		if (entry.bare) return false;
		const resolvedWorktree = resolve(entry.worktree);
		if (resolvedWorktree === mainCheckoutRoot) return false;
		if (resolvedWorktree === resolve(currentTopLevel)) return Boolean(includeCurrent);
		return true;
	});
	const detailed = await Promise.all(
		candidates.map(async (entry) => {
			const metadata = await readWorktreeMetadata(entry.worktree);
			const mainCheckoutPath = metadata?.mainCheckoutPath || currentTopLevel;
			const taskBranch = metadata?.taskBranch || entry.branch || (entry.detached ? "" : await getCurrentBranch(entry.worktree).catch(() => ""));
			const baseBranch = metadata?.baseBranch || (taskBranch ? await inferBaseBranch(mainCheckoutPath, taskBranch) : undefined);
			return {
				worktreePath: entry.worktree,
				branch: taskBranch || entry.branch || "(detached)",
				baseBranch,
				isCurrent: resolve(entry.worktree) === resolve(currentTopLevel),
				hasMetadata: Boolean(metadata),
			};
		}),
	);
	return {
		repoRoot: currentMetadata?.repoRoot || currentTopLevel,
		currentTopLevel,
		candidates: detailed,
	};
}
...
export async function attachWorktree({ cwd, target }) {
	...
	if (normalizedTarget) {
		...
		if (!match && candidates.length === 1) match = candidates[0];
			...
			throw new Error(`Multiple linked worktrees are available. Re-run /worktree attach with a branch or path.\n\nCandidates:\n${summary}`);
		}
	}
	...
	await writeWorktreeMetadata(state);
	const includeResult = await applyConfiguredIncludes({ sourceRoot: mainCheckoutPath, worktreePath: match.worktree });
	return attachIncludeResult(state, includeResult);
}
...
export async function finishWorktree(state, options = {}) {
	...
	if (initialStatus.rebaseInProgress || initialStatus.mergeInProgress) {
		return {
			status: "blocked",
			message: "Worktree already has a merge or rebase in progress. Resolve that first, then rerun /worktree finish.",
	...
	if (strategy === "auto") {
		...
		if (rebaseResult.code !== 0) {
			return {
				status: "conflict",
				message: "Rebase hit conflicts inside the active worktree. Resolve them in this pi session, then rerun /worktree finish.",
...
export function getWorktreeAttachTarget(candidate) {
	return candidate.branch && candidate.branch !== "(detached)" ? candidate.branch : candidate.worktreePath;
}

export function formatWorktreeCandidate(candidate) {
	const current = candidate.isCurrent ? " [current checkout]" : "";
	const base = candidate.baseBranch ? ` -> ${candidate.baseBranch}` : "";
	return `${candidate.branch}${base}: ${candidate.worktreePath}${current}`;
}

export function formatWorktreeListText({ repoRoot, currentTopLevel, candidates }) {
	const lines = [`repo: ${repoRoot}`];
	if (candidates.length === 0) {
		lines.push("linked worktrees: none");
		return lines.join("\n");
	}
	lines.push(`linked worktrees: ${candidates.length}`);
	for (const candidate of candidates) {
		lines.push(`- ${formatWorktreeCandidate(candidate)}`);
	}
	if (resolve(currentTopLevel) !== resolve(repoRoot)) {
		lines.push(`current checkout: ${currentTopLevel}`);
	}
	return lines.join("\n");
}

export function formatAttachableStatusText({ repoRoot, currentTopLevel, candidates }) {
	const lines = [`repo: ${repoRoot}`];
	if (candidates.length === 0) {
		lines.push("no active worktree flow for this session");
		lines.push("attachable worktrees: none");
		return lines.join("\n");
	}
	lines.push("no active worktree flow for this session");
	lines.push(`attachable worktrees: ${candidates.length}`);
	for (const candidate of candidates) {
		lines.push(`- ${formatWorktreeCandidate(candidate)}`);
	}
	if (candidates.length === 1) {
		const candidate = candidates[0];
		lines.push(`hint: run /worktree attach ${getWorktreeAttachTarget(candidate)}`);
	} else {
		lines.push("hint: run /worktree attach <branch-or-path>");
	}
	if (resolve(currentTopLevel) !== resolve(repoRoot)) {
		lines.push(`current checkout: ${currentTopLevel}`);
	}
...
export function detachWorktree(state) {
	return {
		status: "success",
		message: `Detached this pi session from ${state.worktreePath}. The worktree still exists and can be re-attached later.`,
		details: {
