...
import { access, glob, lstat, mkdir, readFile, readlink, rm, symlink, writeFile } from "node:fs/promises";
...
function validateIncludePattern(pattern) {
	if (typeof pattern !== "string" || !pattern.trim()) return undefined;
	const trimmed = pattern.trim();
	if (isAbsolute(trimmed)) throw new Error(`${CONFIG_FILE} include entries must be relative paths: ${trimmed}`);
	if (normalize(trimmed).split(/[\\/]+/).includes("..")) {
		throw new Error(`${CONFIG_FILE} include entries cannot contain '..' path segments: ${trimmed}`);
	}
	return trimmed;
}
...
async function readEzWorktreeConfig(repoRoot) {
	...
	const include = Array.isArray(parsed?.include) ? parsed.include.map(validateIncludePattern).filter(Boolean) : [];
	return { include };
}

async function expandIncludePatterns(sourceRoot, patterns) {
	const seen = new Set();
	const files = [];
	for (const pattern of patterns) {
		...
		for await (const entry of glob(pattern, { cwd: sourceRoot })) {
			matched = true;
			const relativePath = validateIncludePattern(entry);
		...
		if (!matched) files.push({ unmatched: pattern });
	}
	return files;
}
...
async function applyConfiguredIncludes({ sourceRoot, worktreePath, config }) {
	...
	if (config.include.length === 0) return result;
	const expanded = await expandIncludePatterns(sourceRoot, config.include);
		...
		if (existing) {
			if (existing.isSymbolicLink()) {
				...
				if (target === sourcePath) {
					result.included.push(relativePath);
					continue;
				}
				await rm(destinationPath);
			} else {
				result.warnings.push(`Skipped include because destination already exists: ${relativePath}`);
				continue;
			}
		}
