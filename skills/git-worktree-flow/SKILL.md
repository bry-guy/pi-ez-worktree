...
## Notes
- `/worktree <subcommand>` is the user-facing slash command. Common subcommands are `start`, `attach`, `detach`, `list`, `status`, `finish`, `abort`, and `help`.
- When you suggest commands to the user, use `/worktree ...` forms and do not suggest the older `/ezwt` or `/wt-*` command names.
- If the user asks what is available, point them to `/worktree help` or the relevant `/worktree help <subcommand>` output.
